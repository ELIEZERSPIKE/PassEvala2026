...
interface FlashInfoFeedProps {
  onArticleClick: (title: string, content: string) => void;
}
  ...
  return (
    <div className="bg-white border border-[#E5E5E5] rounded-sm overflow-hidden flex flex-col shadow-sm" id="component-flash-info">
      ...
      <div className="flex flex-col divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
        {flashInfos.map((item) => (
          <div key={item.id} className="p-4 hover:bg-gray-50 transition-colors cursor-pointer group" onClick={() => onArticleClick(item.title, item.link || 'Aucun contenu détaillé.')}>
