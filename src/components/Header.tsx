...
import getImageUrl from '../utils/imageUtils';   // ← Import important
  ...
  return (
    ...
    >
      <div className="w-full h-[72px] flex items-center justify-between px-4 md:px-8 max-w-7xl mx-auto">
        ...
        {/* Widgets + Auth */}
        <div className="flex items-center gap-3">
          {/* Visiteurs et Météo (inchangés) */}
          <motion.div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(59, 130, 246, 0.2)', color: '#93A8C9', backdropFilter: 'blur(10px)' }} whileHover={{ scale: 1.05 }}>
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
            </span>
            <Eye className="w-3.5 h-3.5" />
            <motion.span key={visitors} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
              {visitors.toLocaleString()}
            </motion.span>
            <span className="text-[9px] uppercase tracking-wider ml-0.5">live</span>
          </motion.div>

          <motion.div className="hidden xl:flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(59, 130, 246, 0.2)', color: '#93A8C9', backdropFilter: 'blur(10px)' }} whileHover={{ scale: 1.05 }}>
            <motion.div animate={{ rotate: weatherCondition === 'sunny' ? 360 : 0 }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}>
              {weatherCondition === 'sunny' ? <Sun className="w-3.5 h-3.5" style={{ color: '#60A5FA' }} /> : <Cloud className="w-3.5 h-3.5" style={{ color: '#93A8C9' }} />}
            </motion.div>
            <span className="text-[9px] uppercase tracking-wider">KARA</span>
            <motion.span key={weatherTemp} initial={{ scale: 1.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ color: '#60A5FA', fontWeight: 800 }}>
              {weatherTemp}°C
            </motion.span>
          </motion.div>
            ...
            {isAuthenticated && user ? (
              ...
              <div className="flex items-center gap-2">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Link to="/login" className="px-4 py-2 text-xs font-bold rounded-xl transition-all" style={{ border: '1px solid rgba(59, 130, 246, 0.4)', color: '#60A5FA', background: 'rgba(59, 130, 246, 0.05)' }}>
                    Connexion
                  </Link>
                </motion.div>
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Link to="/signup" className="px-4 py-2 text-xs font-bold rounded-xl" style={{ background: 'linear-gradient(135deg, #2563EB, #3B82F6)', color: '#FFFFFF', boxShadow: '0 4px 15px rgba(37, 99, 235, 0.3)' }}>
                    S'inscrire
                  </Link>
                </motion.div>
              </div>
          ...
          <motion.button
            ...
            style={{ border: '1px solid rgba(59, 130, 246, 0.3)', background: 'rgba(59, 130, 246, 0.1)', backdropFilter: 'blur(10px)' }}
          >
            {mobileMenuOpen ? <X className="w-5 h-5" style={{ color: '#60A5FA' }} /> : <Menu className="w-5 h-5" style={{ color: '#60A5FA' }} />}
          </motion.button>
        </div>
      </div>
                ...
                {isAuthenticated && user ? (
                  <>
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.05)' }}>
                      <div className="w-10 h-10 rounded-2xl overflow-hidden border border-blue-500/30 flex-shrink-0">
                        {avatarUrl ? (
                          <img 
                            src={avatarUrl} 
                            alt={user.username}
                          ...
                          <div className="w-full h-full bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-lg font-black text-white">
                            {user.username?.slice(0, 2).toUpperCase() || '??'}
                          </div>
                        )}
                      </div>
                      <div>
                        <p className="font-semibold" style={{ color: '#F0F7FF' }}>{user.name || user.username}</p>
                        <p className="text-xs" style={{ color: '#60A5FA' }}>@{user.username}</p>
                      </div>
                    </div>
                    ...
                    {[
                      ...
                      { to: '/profile/likes', label: 'Mes likes' },
                      { to: '/profile/shorts', label: 'Mes shorts' },
                      { to: '/profile/avatar', label: 'Changer avatar' },
                    ].map((link) => (
                      <Link
                        key={link.to}
                        to={link.to}
                        onClick={() => setMobileMenuOpen(false)}
                        className="flex items-center gap-3 w-full py-2.5 px-4 text-sm font-semibold rounded-xl"
                        style={{ color: '#93A8C9', background: 'rgba(255,255,255,0.03)' }}
                      >
                        {link.label}
                      </Link>
                    ))}
                    ...
                    {isStaff && (
                      <Link to="/admin" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 w-full py-3 px-4 text-sm font-bold rounded-xl" style={{ color: '#60A5FA', background: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.2)' }}>
                        Administration
                      </Link>
                    )}

                    <button onClick={() => { handleLogout(); setMobileMenuOpen(false); }} className="flex items-center gap-3 w-full py-3 px-4 text-sm font-bold rounded-xl text-left" style={{ color: '#F87171', background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                      Déconnexion
                    </button>
                  </>
                ) : (
                  <>
                    <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-center w-full py-3 px-4 text-sm font-bold rounded-xl" style={{ border: '1px solid rgba(59, 130, 246, 0.4)', color: '#60A5FA', background: 'rgba(59, 130, 246, 0.05)' }}>
                      Connexion
                    </Link>
                    <Link to="/signup" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-center w-full py-3 px-4 text-sm font-bold rounded-xl" style={{ background: 'linear-gradient(135deg, #2563EB, #3B82F6)', color: '#FFFFFF', boxShadow: '0 4px 15px rgba(37, 99, 235, 0.3)' }}>
                      S'inscrire
                    </Link>
                  </>
                )}
...
//           {/* ✅ Auth Desktop — Dropdown ou boutons */}
//           <div className="hidden lg:flex items-center">
//             {isAuthenticated && user ? (
//               <UserDropdown />
//             ) : (
...
//                   <>
//                     {/* Infos user */}
//                     <div className="flex items-center gap-2 px-4 py-2 text-xs font-semibold" style={{ color: '#F0F7FF' }}>
//                       <div
