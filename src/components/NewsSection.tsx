...
export default function NewsSection({ onArticleClick }: NewsSectionProps) {
  ...
  // Écoute les mises à jour de likes
  useEffect(() => {
    const handleLikeUpdate = (e: Event) => {
      const { articleId, isLiked, likesCount } = (e as CustomEvent).detail;
      setArticles(prev => prev.map(a => 
        a.id === articleId 
          ? { ...a, is_liked_by_user: isLiked, likes_count: likesCount } 
          : a
      ));
    };
  ...
  // Écoute les mises à jour du nombre de commentaires
  useEffect(() => {
    const handleCommentUpdate = (e: Event) => {
      const { articleId, commentsCount } = (e as CustomEvent).detail;
      setArticles(prev => prev.map(a => 
        a.id === articleId 
    ...
    window.addEventListener('article-comment-updated', handleCommentUpdate);
    return () => window.removeEventListener('article-comment-updated', handleCommentUpdate);
  }, []);
