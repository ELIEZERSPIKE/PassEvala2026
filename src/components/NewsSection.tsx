...
export default function NewsSection({ onArticleClick }: NewsSectionProps) {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const data = await articleService.getAll(); // ✅ bonne méthode
        setArticles(data);
      } catch (err) {
          ...
          {heroArticle && (
            ...
            >
              ...
              <div className="p-6 flex flex-col gap-3">
                ...
                <button
                  onClick={() => onArticleClick(heroArticle)}
                  className="bg-blue-600 text-white font-display font-extrabold text-xs uppercase py-2 px-4 rounded-sm mt-2 self-start hover:bg-blue-700"
                >
                  Lire l'Article
                </button>
              </div>
              ...
              <div className="flex flex-col gap-4">
                {listArticles.map((article) => (
                  <div
                    ...
                    className="bg-white border border-[#E5E5E5] rounded-sm p-4 hover:border-gray-400 transition-all duration-200 flex flex-col sm:flex-row gap-4 cursor-pointer group"
                  >
                    <div className="w-full sm:w-32 h-24 bg-gray-100 rounded-sm overflow-hidden flex-shrink-0">
                      <img
                    ...
                    <div className="flex flex-col justify-between flex-grow gap-2">
                      <div>
                        ...
                        <h3 className="font-display font-extrabold text-sm text-[#222222] group-hover:text-blue-600 transition-colors">
                          {article.title}
                        </h3>
                        <p className="font-sans text-xs text-gray-500 line-clamp-2 mt-1">
                          {article.summary}
                        </p>
                      </div>
                    </div>
