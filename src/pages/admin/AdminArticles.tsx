    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          ...
          {(user?.role === 'superadmin' || user?.role === 'admin' || hasPermission('manage articles')) && (
            <Link to="/admin/articles/new" className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all flex items-center gap-2">
          ...
          <table className="min-w-full">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">Auteur</th>
              ...
              {articles.map((article) => (
                <tr key={article.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    {article.image_path && (
                      <img src={getImageUrl(article.image_path)} alt="" className="w-14 h-10 object-cover rounded-lg shadow-sm" />
                    )}
                  </td>
