...
export const AdminSponsors: React.FC = () => {
  ...
  const loadSponsors = async () => {
    try {
      setLoading(true);
      // 1. ATTENTION : Utilise la méthode admin du service (ex: getAllSponsors ou index)
      // qui tape sur la route liée à public function index(Request $request)
      const response = await sponsorService.getAllSponsors();

      // 2. CORRECTION CLÉ : Laravel Resource renvoie { data: [...] }
      if (response && response.data && Array.isArray(response.data)) {
        // Si c'est paginé ou une Resource simple, les données sont dans .data
        setSponsors(response.data);
      } else if (Array.isArray(response)) {
        // Si le service a déjà déballé les données
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white shadow-sm border border-gray-100 rounded-[2rem] overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-gray-500 font-medium">Chargement des sponsors...</div>
        ) : sponsors.length === 0 ? (
          <div className="p-12 text-center text-gray-500 font-medium">Aucun sponsor trouvé.</div>
        ) : (
                      ...
                      {sponsor.banner_url ? (
                        <img 
                          ...
                          onError={(e) => {
                                ...
                                console.warn(`Échec du chargement de l'image pour ${sponsor.name}:`, target.src);
                                // Utilisation de placehold.co (plus stable) et arrêt de la boucle d'erreur
                                target.onerror = null; 
                                target.src = 'https://placehold.co/800x400?text=Lien+Invalide';
                          }}
                        />
                      ) : (
