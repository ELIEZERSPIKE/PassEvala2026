...
import ProfileShorts from '../pages/profile/ProfileShorts';
...
const MainLayout = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {/* AppNavbar — uniquement sur /admin/* pour admin/superadmin */}
      {isAdmin && isAdminRoute && <AppNavbar />}

      {/* Navbar publique — hors admin et hors accueil */}
      ...
      <main className="flex-grow">
        <Routes>
          ...
          {/* ─── Routes profil (authentifié requis) ───────────────────── */}
          <Route
            path="/profile/me"
            element={isAuthenticated ? <ProfileMe /> : <Navigate to="/login" replace />}
          />
          <Route
            path="/profile/likes"
            element={isAuthenticated ? <ProfileLikes /> : <Navigate to="/login" replace />}
          />
          <Route
            path="/profile/shorts"
            element={isAuthenticated ? <ProfileShorts /> : <Navigate to="/login" replace />}
          />
          <Route
            path="/profile/avatar"
            element={isAuthenticated ? <ProfileAvatar /> : <Navigate to="/login" replace />}
          />
      ...
      {/* Footer — hors admin, hors accueil, hors profil */}
      {!isAdminRoute && !isHomePage && !isProfileRoute && <Footer />}
    </div>
  );
};
...
const BonPlanEditWrapper = () => {
  ...
  useEffect(() => {
    if (id) {
      bonPlanService
        .getOne(Number(id))
        .then(data => { if (data) setBonPlan(data); })
        .catch(console.error);
    }
  }, [id]);
...
//       {/* Footer — uniquement hors admin et hors accueil */}
